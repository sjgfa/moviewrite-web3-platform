...
async function main() {
  console.log("🔑 Hardhat 测试账户信息 (含私钥)");
  ...
  for (let i = 0; i < Math.min(testAccounts.length, 10); i++) {
    const testAccount = testAccounts[i];
    let balance = "N/A ETH";
    
    try {
      if (accounts[i]) {
        const bal = await accounts[i].provider.getBalance(testAccount.address);
    ...
    console.log(`账户 ${i} (${testAccount.role}):`);
  ...
  console.log("2. 点击账户头像 → 导入账户");
  console.log("3. 选择 '私钥' 导入方式");
  console.log("4. 复制粘贴上面任意一个私钥");
  console.log("5. 添加本地网络:");
